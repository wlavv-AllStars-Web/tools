...
class dpdController extends Controller
{
    ...
    public function generateCSV($id_order, $weight){
        ...
        if ($csv = fopen($path.$file, 'w')) {
            ...
            $includeCompany = (int) $order->id_shop !== 3;

            $data[] = self::dpdText(self::recipientName($order->delivery, $includeCompany));
            ...
            if( ($order->recyclable == 1 ) ){
                ...
                $data[] = self::dpdText(self::recipientName($order->invoice, $includeCompany));
    ...
    private static function recipientName($address, bool $includeCompany = true): string
    {
        $company = trim((string) ($address->company ?? ''));
        $company = trim($company, " \t\n\r\0\x0B|-/");
        $name = trim((string) ($address->firstname ?? '') . ' ' . (string) ($address->lastname ?? ''));

        return $includeCompany && $company !== '' ? trim($name . ' (' . $company . ')') : $name;
    }
