...
@section('content')
    ...
    <div class="navbar navbar-light customPanel">
        <table class="currency-table">
            <thead>
                <tr>
                    <th colspan="5">ACTIVE CURRENCY CONVERSION BY STORE</th>
                </tr>
                <tr>
                    <th></th>
            ...
            <tbody>
                @foreach($rates as $provider => $currencies)
                    <tr>
                        <td class="currency-provider" style="text-align: right;">
                            <div style="margin: 0 5px;">{{ $provider }}</div>
                        </td>
                        @foreach($symbols as $code => $symbol)
                        <td style="border-left: 1px solid #999">
                            <table style="width: 100%;">
                                <tr>
                                    <td style="border-bottom: 0px solid #000;">
                                        <span style="font-weight: bolder;">P: </span>
                                        <span style="color: darkgreen;">
                                            {!! isset($currencies[$code]['purchase']) ? number_format((float)$currencies[$code]['purchase'], 4, '.', ',') : '&mdash;' !!}
                                        </span>
                                    </td>
                                    <td style="border-bottom: 0px solid #000;">
                                        <span style="font-weight: bolder;">S: </span>
                                        <span style="color: red;">
                                            {!! isset($currencies[$code]['sale']) ? number_format((float)$currencies[$code]['sale'], 4, '.', ',') : '&mdash;' !!}
                                        </span>
                                    </td>
                                </tr>
                            </table>
