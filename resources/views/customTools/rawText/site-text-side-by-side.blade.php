...
@section('content')
    <style>
        ...
        textarea{min-width:95%;max-width:100%;min-height:80px;border:1px solid #d7dde5;border-radius:10px;padding:12px;font-family:monospace;font-size:14px}
        ...
        .empty{background-image:linear-gradient(45deg,#f5f5f5 25%,transparent 25%,transparent 50%,#f5f5f5 50%,#f5f5f5 75%,transparent 75%,transparent);background-size:16px 16px;min-height:38px}
        ...
        .table-wrap{overflow:auto;max-height:72vh;border:1px solid #e5e7eb;border-radius:12px}
        .lang-badge{display:inline-flex;align-items:center;gap:10px;font-size:50px;font-weight:700}
        .lang-code{font-size:14px;color:#475467;text-transform:uppercase;background:#f2f4f7;border:1px solid #e4e7ec;padding:4px 10px;border-radius:999px}
        ...
        .diff-removed{background:#ffd7d7;padding:1px 2px;border-radius:4px}
        .diff-added{background:#d8f5d8;padding:1px 2px;border-radius:4px}
    ...
    <div class="wrap" style="margin-top: 10px;">
        <div class="card" style="text-align:center;">
            ...
            <p class="muted">
                ...
                <span class="mono">lang|url_old|url_new</span>
            </p>
            <form method="post" action="{{ route('site-text-side-by-side.compare') }}">
                @csrf
...
es|https://site-antigo.com/es/pagina-a|https://site-novo.com/es/pagina-a">{{ old('pairs', $input ?? '') }}</textarea>
                <br><br>
                <button type="submit">Comparar lado a lado</button>
            </form>
        </div>
                    ...
                    @else
                        <div class="table-wrap">
                            <table>
                                <thead>
                                    <tr>
                                        <th style="width:47%">Texto site antigo</th>
                                        <th style="width:6%">%</th>
                                        <th style="width:47%">Texto site novo</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    @foreach($result['rows'] as $row)
                                        @php
                                            $leftCell = ($row['left'] ?? '') !== ''
                                                ? (($row['left_html'] ?? '') !== '' ? nl2br($row['left_html']) : nl2br(e($row['left'])))
                                                : '<div class="empty">&nbsp;</div>';

                                            $rightCell = ($row['right'] ?? '') !== ''
                                                ? (($row['right_html'] ?? '') !== '' ? nl2br($row['right_html']) : nl2br(e($row['right'])))
                                                : '<div class="empty">&nbsp;</div>';
                                        @endphp
