                        ...
                        <a class="oms-supplier-row {{ $isActive ? 'active' : '' }}"
                           href="{{ route('erp.oms.dashboard', ['supplier_tab' => $supplierTab, 'supplier_id' => $supplier->supplier_id, 'center_tab' => $defaultCenterTab]) }}">
                            <div>
                                ...
                                @if($supplierTab === 'all')
                                    <div class="oms-supplier-meta">
                                        O: {{ (int) ($supplier->open_order_notes_count ?? 0) }} /
                                        B: {{ (int) ($supplier->billed_count ?? 0) }} /
                                        R: {{ (int) ($supplier->received_count ?? 0) }}
                                    </div>
                                @endif
                            </div>
                            <span class="badge text-bg-light">
                                ...
                                @elseif($supplierTab === 'billed')
                                    {{ (int) ($supplier->billed_count ?? 0) }}
                                @elseif($supplierTab === 'received')
                                    {{ (int) ($supplier->received_count ?? 0) }}
                                @else
                                    {{ (int) (($supplier->open_order_notes_count ?? 0)+($supplier->billed_count ?? 0)+($supplier->received_count ?? 0)) }}
                                @endif
                            </span>
                        </a>
                    @empty
