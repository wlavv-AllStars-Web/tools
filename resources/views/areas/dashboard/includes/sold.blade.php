<div id="sales_history">
    ...
    <table class="table table-striped table-bordered table-hover" border="1" style="width: 100%; text-align: center; display:none;">
        ...
        <tr>
            @if($data['origin'] == 'ASM')   <td style="color: red;">{{ $data['origin'] }}</td> @endif
            @if($data['origin'] == 'ASD')   <td style="color: dodgerblue">{{ $data['origin'] }}</td> @endif
            @if($data['origin'] == 'CROSS') <td style="color: black;">{{ $data['origin'] }}</td> @endif
