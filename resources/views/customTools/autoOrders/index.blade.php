                ...
                <div>
                    @foreach($auto_orders AS $key => $manufacturer)
                        @if(!in_array($manufacturer['name'], [
                            ...
                            <div class="card" style="border-radius: 0;width: 100%" id="row_brand_{{str_replace(' ', '-', $manufacturer['name'])}}">
                                <div class="card-body" style="padding: 0px;">
                                    <h5 class="card-title" style="margin-bottom: 0;height: 32px;">
                                        <div style="width: calc( 100% - 200px ); float: left; cursor: pointer;" onclick="getProductsInfo('{{addslashes($manufacturer['name'])}}', '{{str_replace(' ', '-', $manufacturer['name'])}}')">
                                        ...
                                        <div style="margin: 0 5px;padding: 5px;float: right;width: 175px;">
                                            <span onclick="cleanBrand('{{addslashes($manufacturer['name'])}}', {{$manufacturer['id_manufacturer']}})" target="_blank" style="cursor: pointer;text-decoration: none; float: right;color: #333; margin-right: 5px;">{{ __("messages.CLEAN")}}</span>
                                            <span style="margin: 0 5px;float: right;">|</span>
                                            <span onclick="createAndDownloadZip('{{addslashes($manufacturer['name'])}}')" style="text-decoration: none; float: right;color: #333;cursor: pointer;" download>{{ __("messages.EXPORT")}}</span>
                                        </div>
                                    </h5>
                                    <div style="border-top: 1px solid #bbb; display: none;" id="table_{{str_replace(' ', '-', $manufacturer['name'])}}"> </div>
                                </div>
                            </div>
                        @endif
                    @endforeach
