...
@section('content')
    ...
    <div class="navbar navbar-light customPanel" style="max-width: 500px;">
        ...
        <div style="width: 100%;display: none;" id="mainContainer">
            ...
            <div id="entryStockContainer" style="width: 100%;display: none;">
                ...
                <div style="text-align:center;"> 
                    <span style="padding: 5px 0; font-size: 18px;">{{ __("messages.ORDER REFERENCE:")}} </span> 
                ...
                <div style="width: 100%;margin-top: 10px;">
                    ...
                    <span style="float: left;">{{ __("messages.INFO")}}</span>
                    ...
                    <div class="collapse" id="collapseExample" style="margin-top: 10px;">
                        <div class="card card-body" style="background-color: #eee">
                            <table style="width: 400px;margin: 0 auto;">
                                <tr>
                                    <td style="width: 100px;text-align: right;">{{ __("messages.NEW:")}}</td>
                                    <td style="text-align: left;" id="tag_new"></td>
                                </tr>
                                <tr>
                                    <td style="width: 100px;text-align: right;">{{ __("messages.LOCATION:")}}</td>
                                    <td style="text-align: left;" id="tag_location"></td>
                                </tr>
                                <tr>
                                    <td style="width: 100px;text-align: right;">{{ __("messages.BACKORDER:")}}</td>
                                    <td style="text-align: left;" id="container_backorders"></td>
                                </tr>
                                <tr>
                                    <td style="width: 100px;text-align: right;">{{ __("messages.PARTIAL:")}}</td>
                                    <td style="text-align: left;" id="container_partials"></td>
                                </tr>
                                <tr>
                                    <td style="width: 100px;text-align: right;">{{ __("messages.PREPARATIONS:")}}</td>
                ...
                <div class="text-center" style="padding: 5px;margin: 5px 0;">
                        <span style="font-size: 18px;color: #777;">{{ __("messages.WEIGHT / MEASUREMENTS")}}</span> <br>
                ...
                <div style="display: none;width: 100%;" id="set_measures">
                    <hr>
                    <button class="btn btn-grey" type="button" data-bs-toggle="collapse" data-bs-target="#collapseExample1" aria-expanded="false" aria-controls="collapseExample1" style="width: 100%;font-weight: bolder; font-size: 18px;color: red;margin-top: 30px 0;background-color: #ddd;">{{ __("messages.WEIGHT / MEASUREMENTS")}}</button>
                        ...
                        <div class="card card-body" style="background-color: #eee">
                            <form name="measurementsForm" id="measurementsForm" method="POST" action="#">
                                ...
                                <table style="max-width: 100%;margin: 0 auto;">
                                    ...
                                    <tr style="height: 40px;">
                                        <td style="width: 40%;text-align: right;">{{ __("messages.MEASUREMENTS ( cm ):")}}</td>
                                        <td> 
                                    ...
                                    <tr style="height: 40px; text-align: center;">
                                        <td colspan="2"> 
                                            <button class="btn btn-info" type="button" onclick="saveMeasurementForm()" style="width: 100%; margin: 0 auto; text-transform: uppercase; font-weight: bolder; font-size: 18px;">{{ __("messages.SAVE")}}</button>
                                        </td>
                                    </tr>
                                </table>   
                            </form>                                 
                ...
                <div style="display: inline-block;width: 100%;text-align: center;">
                    <table style="width: 100%;">
                        ...
                        <tr style="height: 60px;">
                            <td style="width: 30%">
                                <div>{{ __("messages.ORDERED")}}</div>
                                <div id="tag_ordered">0</div>
                            </td>
                            <td style="width: 30%">
                                <div>{{ __("messages.BILLED")}}</div>
                                <div id="tag_billed">0</div>
                            </td>
                            <td style="width: 40%">
                                <div>
                                    <div style="margin-top: 10px;">{{ __("messages.RECEIVED")}}</div>
                        ...
                        <tr style="height: 35px;" id="saveEntryContainer">
                            <td colspan="4"><button type="button" onclick="saveStockEntry($(this))" class="btn btn-success" style="width: calc( 250px);font-size: 18px;margin: 10px 0 0 0;">{{ __("messages.SAVE STOCK ENTRY")}}</button></td>
                        </tr>
                    </table>
                </div>
                <div style="width: 100%;">
                    <hr>
                    <button class="btn btn-warning" type="button" style="width: 48%; text-transform: uppercase; font-size: 18px; float: left; color: #FFF;" onclick="updateEAN()">{{ __("messages.UPDATE EAN13")}}</button>
                    <button class="btn btn-info"    type="button" style="width: 48%; text-transform: uppercase; font-size: 18px; float: right;color: #FFF;" onclick="report()">{{ __("messages.REPORT")}}</button>
                </div>
                <div style="width: 100%; display: none;" id="reportsContainer">
                    <hr>
                    <div id="containerReportButton">
                        <div>
                            <div style="text-align: center;">{{ __("messages.NOTES")}}</div>    
                        ...
                        <button class="btn btn-secondary" type="button" style="text-transform: uppercase; width: calc( 50% - 10px ); float: left; margin: 5px;" onclick="sendReport('DAMAGED PRODUCT')">{{ __("messages.DAMAGED PRODUCT")}}</button>
                        <button class="btn btn-secondary" type="button" style="text-transform: uppercase; width: calc( 50% - 10px ); float: left; margin: 5px;" onclick="sendReport('BOX SIZE DIFFERENT')">{{ __("messages.BOX SIZE DIFFERENT")}}</button>
                        <button class="btn btn-secondary" type="button" style="text-transform: uppercase; width: calc( 50% - 10px ); float: left; margin: 5px;" onclick="sendReport('EAN-13 HAVE CHANGED')">{{ __("messages.EAN-13 HAVE CHANGED")}}</button>
                        <button class="btn btn-secondary" type="button" style="text-transform: uppercase; width: calc( 50% - 10px ); float: left; margin: 5px;" onclick="sendReport('REFERENCE HAVE CHANGED')">{{ __("messages.OPERATOR")}}</button>
                    </div>
        ...
        <div style="display: none;width: 100%;" id="mainContainerEmpty">
            <div class="alert alert-danger" style="margin: 20px 10px;">{{ __("messages.NO DATA FOUND!")}}</div>
        </div>
    </div>
