                ...
                <div id="collapse-{{ $section['id'] }}" class="accordion-collapse collapse" data-bs-parent="#omsSearchAccordion">
                    <div class="accordion-body p-0">
                        ...
                        @elseif($section['id'] === 'order-notes')
                            ...
                            @foreach($section['rows'] as $row)<tr><td><a href="{{ route('erp.oms.simple', ['supplier_id' => $row->supplier_id, 'document_scope' => $row->document_status === 'closed' ? 'closed' : 'open', 'order_note_id' => $row->order_note_id]) }}">{{ $row->document_reference }}</a></td><td>{{ $row->search_reference }}</td><td>{{ $row->search_product_name }}</td><td class="text-center">{{ $row->qty_ordered }}</td><td>{{ $row->document_status }}</td><td>{{ $row->document_date }}</td></tr>@endforeach
                            </tbody></table></div>
                        @elseif($section['id'] === 'billed-notes')
                            ...
                            @foreach($section['rows'] as $row)<tr><td><a href="{{ route('erp.oms.simple', ['supplier_id' => $row->supplier_id, 'document_scope' => $row->order_note_status === 'closed' ? 'closed' : 'open', 'order_note_id' => $row->order_note_id]) }}">{{ $row->document_reference }}</a></td><td>{{ $row->invoice_reference ?: '-' }}</td><td>{{ $row->search_reference }}</td><td>{{ $row->search_product_name }}</td><td class="text-center">{{ $row->qty_billed }}</td><td>{{ $row->document_date }}</td></tr>@endforeach
                            </tbody></table></div>
                        @elseif($section['id'] === 'received-notes')
                            ...
                            @foreach($section['rows'] as $row)<tr><td><a href="{{ route('erp.oms.simple', ['supplier_id' => $row->supplier_id, 'document_scope' => $row->order_note_status === 'closed' ? 'closed' : 'open', 'order_note_id' => $row->order_note_id]) }}">{{ $row->document_reference }}</a></td><td>{{ $row->invoice_reference ?: '-' }}</td><td>{{ $row->search_reference }}</td><td>{{ $row->search_product_name }}</td><td class="text-center">{{ $row->qty_received }}</td><td>{{ $row->document_date }}</td></tr>@endforeach
                            </tbody></table></div>
